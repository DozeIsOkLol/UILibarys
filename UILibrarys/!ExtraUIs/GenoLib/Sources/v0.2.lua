-- GENOUI Library + Full Example (All-in-One)
-- DM Callum#0001 for support (don't forget the dot at the end!)
-- Sunderland finest UI library - v0.1 BETA - January 2026

local GENOUI = {}
GENOUI.__index = GENOUI
...
local function setupButtonHover(button, normalColor, hoverColor)
    ...
    button.MouseEnter:Connect(function() tween:Play() end)
    button.MouseLeave:Connect(function() reverseTween:Play() end)
end
...
function GENOUI:CreateWindow(options)
    local self = setmetatable({}, Window)
    
    self.Title    = options.Title    or "GENO"
    self.Subtitle = options.Subtitle or "Callum's Hub - Sunderland"
    self.Size     = options.Size     or UDim2.new(0, 620, 0, 460)
    
    self.ScreenGui = Instance.new("ScreenGui")
    self.ScreenGui.Name = "GENOUI"
    ...
    self.BadgesContainer.Position = UDim2.new(1, -200, 0, 10)  -- moved to right side
    self.BadgesContainer.AnchorPoint = Vector2.new(1, 0)      -- anchor from right
...
function Window:AddTab(name)
    ...
    tab.AddDropdown = function(_, options)
        ...
        dropdownText.Text = options.Name or "Dropdown"
        ...
        dropdownButton.Position = UDim2.new(1, -90, 0.5, -15)
        ...
        createUIStroke(dropdownButton, Color3.fromRGB(80, 80, 80))
        dropdownButton.Parent = dropdownFrame
        setupButtonHover(dropdownButton, Color3.fromRGB(60, 60, 60), Color3.fromRGB(80, 80, 80))
        setupButtonClick(dropdownButton, Color3.fromRGB(60, 60, 60))
        ...
        dropdownOptions.Parent = dropdownButton
    ...
    tab.AddInformation = function(_, options)
        ...
        createUIStroke(infoFrame, Color3.fromRGB(60, 60, 60))
        ...
        infoIcon.Image = "rbxassetid://7072718266" -- Info icon
        infoIcon.Parent = infoFrame
        
        local infoTitle = Instance.new("TextLabel")
        infoTitle.Size = UDim2.new(1, -50, 0, 20)
        infoTitle.Position = UDim2.new(0, 50, 0, 5)
        infoTitle.BackgroundTransparency = 1
        infoTitle.Text = options.Title or "Information"
    ...
    tab.AddWarning = function(_, options)
        ...
        createUICorner(warningFrame, 6)
        createUIStroke(warningFrame, Color3.fromRGB(60, 60, 60))
...
-- =============================================================================
-- EXAMPLE UI – runs when script is executed
-- =============================================================================

do
    local Window = GENOUI:CreateWindow({
        Title    = "GENO UI",
        Subtitle = "Callum's Sunderland Special - v0.1 BETA",
        Size     = UDim2.new(0, 620, 0, 460)
    })
    

        -- Badges on the right side
    Window:AddBadge("Alpha",    Color3.fromRGB(180, 100, 255))   -- purple-ish
    Window:AddBadge("v0.1",     Color3.fromRGB(0, 170, 255))     -- blue
    Window:AddBadge("BETA",     Color3.fromRGB(255, 140, 0))     -- orange
    Window:AddBadge("Release",  Color3.fromRGB(0, 200, 80))      -- green
    Window:AddBadge("Private",  Color3.fromRGB(220, 60, 60))     -- red
    ...
    PlayerTab:AddToggle({
        ...
        Callback = function(v)
            noclip = v
            print("Noclip toggled:", v and "ON" or "OFF")
        end
    })
    ...
    MiscTab:AddButton({
        Name = "Hello Sunderland",
        Callback = function()
            print("From Sunderland with love - GENO UI v0.1 BETA!")
        end
    })
    
    MiscTab:AddParagraph({
        Text = "GENO UI - Callum's custom library\nv0.1 BETA\nMade in Sunderland, England\nUse wisely lads"
    })
    
    SettingsTab:AddInformation({
        Title = "Quick Tip",
        Message = "Drag the top bar to move\nMinimize with - button\nFullscreen with square icon"
    })
    
    SettingsTab:AddWarning({
        Title = "Warning",
        Message = "Some features might get detected\nUse at your own risk - no refunds on bans"
    })
    
    SettingsTab:AddDropdown({
        Name = "Theme",
        Options = {"Dark", "Neon", "Sunderland Red & White"},
        Callback = function(choice)
            print("Theme changed to:", choice)
        end
    })
    
    print("GENO UI v0.1 BETA loaded successfully - enjoy Callum!")
end
