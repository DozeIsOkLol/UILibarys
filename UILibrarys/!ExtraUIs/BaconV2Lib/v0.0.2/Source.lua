--[[
    Glow Pro UI Library v1.0.3 (Syntax Hotfix)
...
for _, v in pairs(CGUI:GetChildren()) do
    if v.Name == "_GLOWPRO_ROOT" or v.Name == "_GLOWPRO_NOTIFICATIONS" then
        v:Destroy()
    end
end
...
function GlowPro:CreateWindow(title, keybind)
    ...
    screenGui.DisplayOrder = 998 -- Lower than notifications
    ...
    -- Top Padding
    local topPadding = Instance.new("Frame", container)
    topPadding.Size = UDim2.new(1,0,0,GlowPro.Theme.WindowPadding / 2)
    ...
    function window:AddSlider(name, min, max, start, callback)
        ...
        bar.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then isSliding = true end end)
...
local function getNotificationContainer()
    ...
    screenGui.Name = "_GLOWPRO_NOTIFICATIONS"
    ...
    container.Position = UDim2.new(1, -290, 0, 0) -- Positioned in bottom right
...
function GlowPro:CreateNotification(title, text, duration)
    ...
    frame.LayoutOrder = tick() -- Use tick to ensure newest is always on bottom
    frame.Parent = container

    -- Create a UIStroke for a border effect
    local stroke = Instance.new("UIStroke", frame)
    stroke.Color = GlowPro.Theme.Accent
    stroke.Thickness = 1
    ...
    textLabel.Text = text
    textLabel.TextColor3 = GlowPro.Theme.MutedText
    ...
    TweenService:Create(frame, animInfo, {GroupTransparency = 0}):Play()
    
    -- Schedule Destruction
    task.delay(duration, function()
        if not frame.Parent then return end -- Already destroyed
        local exitTween = TweenService:Create(frame, animInfo, {GroupTransparency = 1})
        exitTween:Play()
