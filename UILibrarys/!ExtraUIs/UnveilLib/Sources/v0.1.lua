--[[
    Unveil UI - A Standalone UI Library
    Synthesized and refined by Your AI Assistant.
    
    Features:
    - Draggable Windows
    - Buttons, Toggles, Sliders, Text Inputs, Labels, and Separators
    - Animated Status Updates
    - Keyboard Navigation (Tab to focus, Enter to press)
    - Global Toggle Key (RightControl by default)
]]
...
function UnveilUI:CreateWindow(title, width, height)
    ...
    titleLabel.Size=UDim2.new(1,-10,1,0); titleLabel.Position=UDim2.new(0,10,0,0); titleLabel.BackgroundTransparency=1; titleLabel.Text=title; titleLabel.TextColor3=Color3.fromRGB(255,255,255); titleLabel.Font=Enum.Font.GothamBold; titleLabel.TextSize=18; titleLabel.TextXAlignment=Enum.TextXAlignment.Left
    ...
    self.canvasY, self.focusableElements, self.currentFocusIndex = 5, {}, nil
    
    -- Dragging logic
    local dragging,dragInput,dragStart,startPos=false,nil,nil,nil;dragBar.InputBegan:Connect(function(i)if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then dragging,dragStart,startPos=true,i.Position,self.mainFrame.Position;i.Changed:Connect(function()if i.UserInputState==Enum.UserInputState.End then dragging=false end end)end end);dragBar.InputChanged:Connect(function(i)if i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch then dragInput=i end end);UserInputService.InputChanged:Connect(function(i)if dragging and dragInput and i==dragInput then local d=i.Position-dragStart;self.mainFrame.Position=UDim2.new(startPos.X.Scale,startPos.X.Offset+d.X,startPos.Y.Scale,startPos.Y.Offset+d.Y)end end)
    
    return self
end

function UnveilUI:_UpdateCanvasSize(addition)
    self.canvasY = self.canvasY + addition
    self.contentFrame.CanvasSize = UDim2.new(0, 0, 0, self.canvasY)
end
...
function UnveilUI:AddSlider(opts)
    ...
    UserInputService.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 and isDragging then isDragging=false; if opts.callback then opts.callback(value) end end end)
    table.insert(self.focusableElements, sliderKnob)
    self:_UpdateCanvasSize(50)
...
function UnveilUI:Init(opts)
    ...
    local function setFocus(index) if index and self.focusableElements[index] then self.currentFocusIndex = index; self.focusStroke.Parent = self.focusableElements[self.currentFocusIndex] else self.currentFocusIndex = nil; self.focusStroke.Parent = nil end end
    ...
    UserInputService.InputBegan:Connect(function(i,gpe)
        if not gpe and i.KeyCode == (opts.toggleKey or Enum.KeyCode.RightControl) then toggleUI() end
        ...
        if i.KeyCode == Enum.KeyCode.Tab then
            ...
            if newIndex > #self.focusableElements then newIndex = 1 elseif newIndex < 1 then newIndex = #self.focusableElements end
            setFocus(newIndex)
        elseif i.KeyCode == Enum.KeyCode.Return or i.KeyCode == Enum.KeyCode.Space then
            if self.currentFocusIndex then local el = self.focusableElements[self.currentFocusIndex]; if el:IsA("TextButton") then el.MouseButton1Click:Fire() end end
        end
    end)
...
--// --- Part 2: RNG Pro Tool Script ---

--// --- Game-Specific Setup ---
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local function findRemote(name) local r=ReplicatedStorage:FindFirstChild(name) if not r then warn("Could not find remote:",name) end return r end
local remotes = {AddSpeed = findRemote("AddSpeed"), Rebirth = findRemote("Rebirth"), Dat = findRemote("Dat"), CrateEvent = findRemote("CrateEvent")}

--// --- UI Creation ---
local Window = UnveilUI:CreateWindow("RNG Pro Tool", 320, 550)

Window:AddLabel({text = "Auto Farming", align = Enum.TextXAlignment.Center, textSize = 18})

Window:AddToggle({text = "Auto Win Farm", callback = function(state)
    _G.AutoWinning = state
    if not state then Window:SetStatus("Auto Win Farm stopped.") return end
    
    Window:SetStatus("Auto Win Farm started.")
    task.spawn(function()
        local pads = {"Get1Wins","Get4Wins","Get15Wins","Get40Wins"}
        while _G.AutoWinning do
            for _, padName in ipairs(pads) do
                if not _G.AutoWinning then break end
                -- (Your auto win logic here)
                Window:SetStatus("Farming on pad: " .. padName)
                task.wait(1)
            end
        end
    end)
...
Window:AddSeparator()
Window:AddLabel({text = "Crate Farming", align = Enum.TextXAlignment.Center, textSize = 18})

Window:AddToggle({text = "Crate Farm (Normal)", callback = function(state)
    _G.CrateFarming = state
    if not state then Window:SetStatus("Crate Farm stopped.") return end
    Window:SetStatus("Normal Crate Farm started.")
    -- (Your crate farm logic here)
end})

Window:AddToggle({text = "Crate Farm (Ultimate)", callback = function(state)
    _G.UltimateCrateFarming = state
    if not state then Window:SetStatus("Ultimate Crate Farm stopped.") return end
    Window:SetStatus("Ultimate Crate Farm started.")
    -- (Your ultimate crate farm logic here)
end})

Window:AddSeparator()
Window:AddLabel({text = "Speed Control", align = Enum.TextXAlignment.Center, textSize = 18})

Window:AddInput({
    placeholder = "Enter speed amount...",
    buttonText = "Add",
    callback = function(text)
        local amount = tonumber(text)
        if amount and remotes.AddSpeed then
            remotes.AddSpeed:FireServer(amount)
            Window:SetStatus("Added " .. amount .. " speed.", Color3.fromRGB(80, 255, 80))
        else
            Window:SetStatus("Invalid number.", Color3.fromRGB(255, 80, 80))
        end
    end
})

Window:AddButton({text = "Rebirth", color = Color3.fromRGB(60,60,60), callback = function()
    if remotes.Rebirth then
        remotes.Rebirth:FireServer()
        Window:SetStatus("Rebirth request sent.")
    end
end})

-- Initialize the UI
Window:Init({toggleKey = Enum.KeyCode.End})
