--[[
    Unveil UI - A Standalone UI Library (V1.5)
    All-in-One Script with Example Usage.
    
    - V1.5 FIX: Completely rewrote the 'AddToggle' function to fix a critical scoping issue that caused a 'nil with Text' crash on click. This is the definitive fix for the toggle bug.
...
-- V1.5 FIX: Rewrote the entire function to solve a critical scoping bug.
function UnveilUI:AddToggle(opts)
    ...
    table.insert(self.focusableElements, toggleButton)
...
function UnveilUI:AddSlider(opts)
    ...
    label.Size = UDim2.new(1,0,0,18); label.BackgroundTransparency = 1; label.Text = opts.text; label.TextColor3 = Color3.fromRGB(180,180,180); label.Font = Enum.Font.Gotham; label.TextSize = 14; label.TextXAlignment = Enum.TextXAlignment.Left
...
Window:AddLabel({
    text = "Created with Unveil UI v1.5",
    align = Enum.TextXAlignment.Center,
    textSize = 12,
