...
| `SidebarBg` | Sidebar panel background |
| `SidebarDivider` | Horizontal rule between tab buttons |
| `TabActive` | Active tab button background |
| `TabActiveAccent` | Blue left-edge accent on active tab |
| `TabInactive` | Inactive tab button background |
| `TabHover` | Tab button background on hover |
| `TabText` | Inactive tab label colour |
| `TabTextActive` | Active tab label colour |
