--[[
	================================================================================
	 SmallDarkOneLib                                                       v0.0.3
	================================================================================

	A dark, horrorcore-inspired Roblox UI library — a blood-red "Bloodmoon"
	theme by default, with an eerie violet "Voidspider" alt-theme. A bold,
	punchy display font for the title and a clean monospace for stats/values
	keep it legible and professional while still feeling like it belongs to
	the same visual world as horrorcore album art (cartoon-villain menace,
	not literal gore).
	...
	local Window = SmallDarkOneLib:CreateWindow({
		Title     = "SMALL DARK ONE",          -- big Bangers display title
...
-- Frames in this library are centre-anchored (AnchorPoint 0.5, 0.5) so they
-- open in the middle of the screen. That means a plain Size tween on its own
-- would shrink/grow around the centre and visibly drag the title bar out of
-- place. This keeps the *top edge* fixed by nudging Position.Y to compensate,
-- so minimizing collapses cleanly upward into the title bar instead of
-- jumping — and it re-reads the frame's current position every call, so it
-- still works correctly even if the window was dragged while minimized.
local function ResizeKeepingTopFixed(frame, newHeight, duration)
	local currentSize = frame.Size
	local currentPos = frame.Position
	local heightDelta = (currentSize.Y.Offset - newHeight) / 2
	Tween(frame, {
		Size = UDim2.new(currentSize.X.Scale, currentSize.X.Offset, 0, newHeight),
		Position = UDim2.new(currentPos.X.Scale, currentPos.X.Offset, currentPos.Y.Scale, currentPos.Y.Offset - heightDelta),
	}, duration or 0.25)
end
...
function SmallDarkOneLib:CreateWindow(config)
	...
	minimizeBtn.MouseButton1Click:Connect(function()
		...
		if minimized then
			...
			ResizeKeepingTopFixed(main, 46, 0.25)
			sidebar.Visible = false
			contentArea.Visible = false
		else
			ResizeKeepingTopFixed(main, expandedHeight, 0.25)
