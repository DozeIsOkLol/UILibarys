...
local Themes = {
    ...
    Midnight = {
        Name = "Midnight",
        Accent = Color3.fromRGB(138, 99, 255), -- Purple
...
-- Export config as base64 string
local function ExportConfigString()
    local configJson = HttpService:JSONEncode(ZenWareFlags)
    local base64 = HttpService:EncodeBase64(configJson)
    return base64
end
...
local function TitleIcon(btn)
    ...
    i.ImageRectOffset = Vector2.new(524, 764)
    i.ImageRectSize = Vector2.new(36, 36)
    i.Size = UDim2.new(0, 14, 0, 14)
    i.Position = UDim2.new(1, -17, 0.5, -7)
    i.Rotation = 180
    i.ZIndex = Level
    return i
end
...
local function Tween(obj, props, info)
    -- Use faster tweens in performance mode
...
function UILibrary.Load(Title, GameName, Version, Options)
    ...
    -- Hide main frame if starting in compact mode
    ...
    MinimizeBtn.MouseButton1Click:Connect(
        function()
            ...
            Tween(CompactWidget, {Rotation = 360}, SlowTweenInfo)
        end
    )
                ...
                if TopBarApp then
                    ...
                    if TopBarFrame then
                        ...
                        if LeftFrame then
                            ...
                            TopBarButton.MouseButton1Click:Connect(
                                function()
    ...
    -- Import config
    function TabLibrary.ImportConfig(configString)
        local success = ImportConfigString(configString)
        if success then
            Notify("Config Imported", "Configuration imported successfully!", "success", 3)
            return true
        else
    ...
    function TabLibrary.AddPage(PageName, HasSearch)
        ...
        function PageLibrary.AddParagraph(Title, Text)
            ...
            -- Calculate height
            local textBounds =
                TextService:GetTextSize(Text, 11, MainFont, Vector2.new(DisplayPage.AbsoluteSize.X - 12, 1000))
        ...
        function PageLibrary.AddMultiDropdown(Text, ConfigArray, Callback, FlagName)
            ...
            if FlagName then
                local savedSelections = GetFlag(FlagName, {})
            ...
            -- Update label text based on selections
            ...
            for idx, opt in ipairs(Arr) do
                ...
                checkbox.ZIndex = Level + 1
                checkbox.Parent = optBtn

                if IsSelected(opt) then
                    local checkmark = TickIcon()
                    checkmark.ZIndex = Level + 2
                    ...
                    function()
                        ...
                        if IsSelected(opt) then
                            ...
                            if not checkbox:FindFirstChild("TickIcon") then
                                ...
                                checkmark.Parent = checkbox
                                Tween(checkmark, {ImageTransparency = 0}, FastTweenInfo)
                            end
                        else
        ...
        function PageLibrary.AddProgressBar(Text, InitialProgress)
            ...
            PBLabel.Size = UDim2.new(1, -12, 0, 14)
            ...
            function ProgressController.SetProgress(progress)
                progress = math.clamp(progress, 0, 100)
                Tween(PBFill, {Size = UDim2.new(progress / 100, 0, 1, 0)}, GlobalTweenInfo)
                PBText.Text = math.floor(progress) .. "%"
            end

            function ProgressController.SetText(text)
                PBLabel.Text = text
            end

            function ProgressController.SetColor(color)
                Tween(PBFill, {ImageColor3 = color}, GlobalTweenInfo)
            end
        ...
        function PageLibrary.AddColorPreset(Text, Presets, Callback, FlagName)
            ...
            CPLabel.Size = UDim2.new(1, -12, 0, 14)
            ...
            for idx, preset in ipairs(Presets) do
                local PresetBtn = RoundBox(5)
                PresetBtn.ImageColor3 = preset.Color
                PresetBtn.ZIndex = Level
                PresetBtn.Parent = PresetGrid

                local PresetClick = TextButton("", 11)
                PresetClick.ZIndex = Level + 1
                PresetClick.Parent = PresetBtn

                PresetClick.MouseEnter:Connect(
                    function()
                        Tween(PresetBtn, {Size = UDim2.new(1, 4, 1, 4)}, FastTweenInfo)
                    end
                )

                PresetClick.MouseLeave:Connect(
                    function()
                        Tween(PresetBtn, {Size = UDim2.new(1, 0, 1, 0)}, FastTweenInfo)
                    end
                )

                PresetClick.MouseButton1Click:Connect(
                    function()
        ...
        function PageLibrary.AddColourPicker(Text, DefaultColour, Callback, FlagName)
            ...
            if typeof(DefaultColour) == "table" then
                DefaultColour =
                    Color3.fromRGB(DefaultColour[1] or 255, DefaultColour[2] or 255, DefaultColour[3] or 255)
        ...
        function PageLibrary.AddToggle(Text, Default, Callback, FlagName)
            ...
            TB.MouseButton1Down:Connect(
                function()
                    ...
                    Tween(
                        FR,
                        {BackgroundColor3 = State and CurrentTheme.Accent or CurrentTheme.CardHover},
                        GlobalTweenInfo
                    )
