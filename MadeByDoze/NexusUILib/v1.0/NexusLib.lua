--[[
╔══════════════════════════════════════════════════════════════╗
║              NexusLib  v1.0 — XanBar-style API               ║
║  Flags (GetFlag/SetFlag/ResetAllFlags) · Quick/QuickNotify   ║
║  AddTab("Name", icon) · AddToggle(name, flag, cb) · AddSlider║
║  AddDropdown(name, flag, options, cb) · AddParagraph(t,c)    ║
║  Tab/NewTab · Section/Label/Divider/Paragraph · ESP tab      ║
║  Notify(Content) · AddInput · AddResetButton → ResetAllFlags ║
╚══════════════════════════════════════════════════════════════╝

    -- XanBar-style (3 lines to a hub):
    local Nexus = loadstring(game:HttpGet("RAW_URL"))()
    local win, tab = Nexus:Quick("My Hub", "Combat", "⚔")
    tab:AddToggle("Aimbot", "aim_on", function(v) end)
    tab:AddSlider("FOV", "aim_fov", {10, 150, Default = 60}, function(v) end)
]]

local NexusLib = {}
NexusLib.__index = NexusLib
NexusLib.Version = "1.0"
...
function NexusLib:Notify(opts)
    opts = opts or {}
    local msg = opts.Message or opts.Content or ""
...
function NexusLib:CreateWindow(opts)
    ...
    local subtitle   = opts.Subtitle  or "v0.9"
    local winSize    = opts.Size      or UDim2.new(0, 580, 0, 415)
    local toggleKey  = opts.ToggleKey or Enum.KeyCode.RightShift
    local unloadKey  = opts.UnloadKey or nil  -- e.g. Enum.KeyCode.End to destroy UI
    ...
    UIS.InputBegan:Connect(function(i, gp)
        ...
        if unloadKey and i.KeyCode == unloadKey then
            Tw(win, { Size = UDim2.new(0, winSize.X.Offset, 0, 0) }, SPRING)
            task.delay(0.4, function() pcall(function() root:Destroy() end) end)
        end
    end)
    ...
    function WindowObj:GetFlag(id) return self.Flags and self.Flags[id] end
    function WindowObj:SetFlag(id, val)
        if self._Controls and self._Controls[id] then
            pcall(function() self._Controls[id].Set(self._Controls[id], val) end)
        end
    end
    function WindowObj:ResetAllFlags()
        for id, defaultVal in pairs(self.DefaultFlags or {}) do
            self.Flags[id] = defaultVal
            if self._Controls and self._Controls[id] then
                pcall(function() self._Controls[id].Set(self._Controls[id], defaultVal) end)
            end
        end
        NexusLib:Notify({ Title = "Reset", Message = "All flags reset to defaults.", Type = "Info" })
    end
    ...
    -- Tab shortcuts (like XAN: quick AddTab with preset name/icon)
    function WindowObj:Combat()    return self:AddTab({ Name = "Combat",    Icon = "⚔" }) end
    function WindowObj:Visuals()   return self:AddTab({ Name = "Visuals",   Icon = "👁" }) end
    function WindowObj:ESP()       return self:AddTab({ Name = "ESP",       Icon = "👁" }) end
    function WindowObj:Misc()      return self:AddTab({ Name = "Misc",      Icon = "📦" }) end
    function WindowObj:Settings()  return self:AddTab({ Name = "Settings",  Icon = "⚙" }) end
    function WindowObj:Player()    return self:AddTab({ Name = "Player",    Icon = "🧑" }) end
    function WindowObj:World()     return self:AddTab({ Name = "World",     Icon = "🌍" }) end
    function WindowObj:Movement()  return self:AddTab({ Name = "Movement",  Icon = "🏃" }) end
    function WindowObj:Main()      return self:AddTab({ Name = "Main",      Icon = "🏠" }) end
    function WindowObj:Home()      return self:AddTab({ Name = "Home",      Icon = "🏠" }) end

    WindowObj.Tab = WindowObj.AddTab
    WindowObj.NewTab = WindowObj.AddTab

    -- ── AddTab ────────────────────────────────────────────────────────────────
    function WindowObj:AddTab(tOptsOrName, iconArg)
        local tOpts
        if type(tOptsOrName) == "string" then
            tOpts = { Name = tOptsOrName, Icon = iconArg or "" }
        else
            tOpts = tOptsOrName or {}
        end
        tOpts.Name = tOpts.Name or "Tab"
        tOpts.Icon = tOpts.Icon or ""
        local tabName = tOpts.Name
        local tabIcon = tOpts.Icon
        ...
        local function RegEl(id, ctrl)
            if id then
                ...
                if WindowObj.Flags then
                    local ok, v = pcall(ctrl.Get, ctrl)
                    if ok then
                        WindowObj.Flags[id] = v
                        WindowObj.DefaultFlags[id] = v
                        WindowObj._Controls[id] = ctrl
                    end
                    local os = ctrl.Set
                    if os then
                        ctrl.Set = function(c, val)
                            os(c, val)
                            WindowObj.Flags[id] = val
                        end
                    end
                end
        ...
        -- XanBar-style arg normalizers: (name, flag, callback) or (name, opts, callback) or (opts)
        local function normToggle(name, arg2, arg3)
            if type(name) == "table" then local o = name; o.Id = o.Id or o.Flag; return o end
            local o = { Name = name }
            if type(arg2) == "string" then o.Id = arg2; o.Callback = arg3
            elseif type(arg2) == "function" then o.Callback = arg2
            elseif type(arg2) == "table" then for k, v in pairs(arg2) do o[k] = v end; o.Callback = arg3 end
            o.Id = o.Id or o.Flag; return o
        end
        local function normSlider(name, arg2, arg3, arg4)
            if type(name) == "table" then local o = name; o.Id = o.Id or o.Flag; return o end
            local o = { Name = name }
            if type(arg2) == "string" then o.Id = arg2
                if type(arg3) == "table" then o.Min = arg3.Min or arg3[1]; o.Max = arg3.Max or arg3[2]; o.Default = arg3.Default or arg3.Min; o.Step = arg3.Step or arg3.Increment; o.Suffix = arg3.Suffix end
                o.Callback = arg4 or (type(arg3) == "function" and arg3)
            elseif type(arg2) == "table" then o.Min = arg2.Min or arg2[1]; o.Max = arg2.Max or arg2[2]; o.Default = arg2.Default; o.Step = arg2.Step or arg2.Increment; o.Suffix = arg2.Suffix; o.Callback = arg3 end
            elseif type(arg2) == "function" then o.Min = 0; o.Max = 100; o.Callback = arg2 end
            o.Id = o.Id or o.Flag; return o
        end
        local function normDropdown(name, arg2, arg3, arg4)
            if type(name) == "table" then local o = name; o.Id = o.Id or o.Flag; o.Options = o.Options or o.Items; return o end
            local o = { Name = name }
            if type(arg2) == "string" then o.Id = arg2; o.Options = type(arg3) == "table" and arg3 or {}; o.Callback = arg4 or (type(arg3) == "function" and arg3)
            elseif type(arg2) == "table" and arg2[1] then o.Options = arg2; o.Callback = arg3
            elseif type(arg2) == "table" then for k, v in pairs(arg2) do o[k] = v end; o.Callback = arg3; o.Options = o.Options or o.Items end
            o.Id = o.Id or o.Flag; return o
        end
        local function normButton(name, arg2)
            if type(name) == "table" then return name end
            return { Name = name, ButtonText = name, Callback = type(arg2) == "function" and arg2 or function() end }
        end
        local function normLabel(arg)
            if type(arg) == "table" then return { Text = arg.Text or arg.Name or "" } end
        ...
        function TabObj:AddSection(n)
            local name = type(n) == "table" and (n.Name or n.Text) or n
            return TabObj.AddSectionToFrame(self, name, scrollFrame)
        end
        ...
        function TabObj.AddAccordionToFrame(_, opts, tf, elList)
            ...
            function AccordionObj:AddParagraph(o) return TabObj.AddParagraphToFrame(nil, o, content, childElements) end
            function AccordionObj:AddResetButton(o) return TabObj.AddResetButtonToFrame(nil, o, content, childElements) end
        ...
        function TabObj:AddLabel(o) return TabObj.AddLabelToFrame(self, normLabel(o), scrollFrame, allElements) end

        -- ── Paragraph (title + multiline content card) ─────────────────────
        function TabObj.AddParagraphToFrame(_, opts, tf, elList)
            opts = opts or {}; tf = tf or scrollFrame; elList = elList or allElements
            local title   = opts.Title or ""
            local content = opts.Content or ""
            local lineH   = 16
            local _, nlCount = content:gsub("\n", "\n")
            local contentH = math.max(20, (nlCount + 1) * lineH + 8)
            local totalH  = 24 + contentH

            local card = New("Frame", {
                Name = opts.Name or "Paragraph", Size = UDim2.new(1,0,0,totalH),
                BackgroundColor3 = T.Card, BorderSizePixel = 0,
                LayoutOrder = #tf:GetChildren(), Parent = tf,
            })
            Corner(7, card); Stroke(T.Border, 1, card)
            ...
            New("TextLabel", {
                Text = title, Font = Enum.Font.GothamBold, TextSize = 12, TextColor3 = T.Text,
                BackgroundTransparency = 1, Position = UDim2.new(0,10,0,6),
                Size = UDim2.new(1,-20,0,16), TextXAlignment = Enum.TextXAlignment.Left, Parent = card,
            })
            New("TextLabel", {
                Text = content, Font = Enum.Font.Gotham, TextSize = 11, TextColor3 = T.TextSub,
                BackgroundTransparency = 1, TextWrapped = true,
                Position = UDim2.new(0,10,0,24), Size = UDim2.new(1,-20,0,contentH),
                TextXAlignment = Enum.TextXAlignment.Left, TextYAlignment = Enum.TextYAlignment.Top, Parent = card,
            })
            AttachTip(card, opts.Tooltip)
            return { Frame = card }
        end
        function TabObj:AddParagraph(titleOrOpts, content)
            local opts = type(titleOrOpts) == "table" and titleOrOpts or { Title = titleOrOpts, Content = content or "" }
            return TabObj.AddParagraphToFrame(self, opts, scrollFrame, allElements) end
        ...
        function TabObj:AddButton(name, arg2) return TabObj.AddButtonToFrame(self, normButton(name, arg2), scrollFrame, allElements) end

        -- Reset to defaults button (calls window:ResetAllFlags() like XanBar)
        function TabObj.AddResetButtonToFrame(_, opts, tf, elList)
            opts = opts or {}; tf = tf or scrollFrame; elList = elList or allElements
            return TabObj.AddButtonToFrame(nil, {
                Name = opts.Name or "Reset",
                ButtonText = opts.ButtonText or "Reset to Defaults",
                Callback = function()
                    if WindowObj and WindowObj.ResetAllFlags then WindowObj:ResetAllFlags() else LoadProfile("Default"); NexusLib:Notify({ Title = "Reset", Message = opts.Message or "Reset to defaults.", Type = "Info" }) end
                end,
            }, tf, elList)
        end
        function TabObj:AddResetButton(o) return TabObj.AddResetButtonToFrame(self, o, scrollFrame, allElements) end
        ...
        function TabObj:AddToggle(name, arg2, arg3) return TabObj.AddToggleToFrame(self, normToggle(name, arg2, arg3), scrollFrame, allElements) end
        ...
        function TabObj.AddSliderToFrame(_, opts, tf, elList)
            ...
            local valLbl = New("TextLabel", {
                Text = tostring(val) .. suffix, Font = Enum.Font.GothamBold, TextSize = 11, TextColor3 = T.Accent,
        ...
        function TabObj:AddSlider(name, arg2, arg3, arg4) return TabObj.AddSliderToFrame(self, normSlider(name, arg2, arg3, arg4), scrollFrame, allElements) end
        ...
        function TabObj.AddDropdownToFrame(_, opts, tf, elList)
            ...
            local listMaxHeight = opts.MaxHeight or 200
            local triggerWidth = math.max(120, opts.TriggerWidth or 120)
            ...
            local trigger = New("TextButton", {
                Size = UDim2.new(0, triggerWidth, 0, 26), Position = UDim2.new(1, -triggerWidth - 8, 0.5, -13),
            ...
            local function openList()
                ...
                if not listFrame or not listFrame.Parent then
                    ...
                    local frameHeight = math.min(listMaxHeight, contentHeight)
                    listFrame = New("Frame", {
                        Size = UDim2.new(0, triggerWidth, 0, frameHeight),
                        BackgroundColor3 = T.Surface,
                        BorderSizePixel = 0,
                        ZIndex = 350,
                        Visible = false,
                    ...
                    local listLayout = List(listScroll, nil, 1)
                    ...
                    -- Force canvas to size after layout (fixes scroll when >2 options)
                    task.defer(function()
                ...
                listFrame.ZIndex = 350
                if listFrame:FindFirstChildWhichIsA("ScrollingFrame") then
                    local sf = listFrame:FindFirstChildWhichIsA("ScrollingFrame")
        ...
        function TabObj:AddDropdown(name, arg2, arg3, arg4) return TabObj.AddDropdownToFrame(self, normDropdown(name, arg2, arg3, arg4), scrollFrame, allElements) end
        ...
        function TabObj.AddMultiDropdownToFrame(_, opts, tf, elList)
            ...
            local isOpen = false
            local multiMaxHeight = opts.MaxHeight or 200
            local triggerWidth = math.max(142, opts.TriggerWidth or 142)
            ...
            local dbtn = New("TextButton", {
                ...
                BackgroundColor3 = T.Elevated, Size = UDim2.new(0, triggerWidth, 0, 24),
                Position = UDim2.new(1, -triggerWidth - 8, 0.5, -12), BorderSizePixel = 0, Parent = card,
            })
            Corner(6, dbtn); Stroke(T.Border, 1, dbtn)

            -- Outer frame (fixed size, clips content)
            local menuHeight = math.min(multiMaxHeight, #items * 30 + 10)
            local menu = New("Frame", {
                Size = UDim2.new(0, triggerWidth, 0, menuHeight),
                BackgroundColor3 = T.Surface,
                BorderSizePixel = 0,
                ZIndex = 350,
                Visible = false,
            ...
            Corner(7, menu); Stroke(T.BorderLight, 1, menu)

            -- ScrollingFrame so all options are clickable when there are many
            ...
            Pad(3, 3, 3, 3, menuScroll)
            local menuListLayout = List(menuScroll, nil, 1)
            ...
            for i, item in ipairs(items) do
                ...
                local chk = New("Frame", {
                    ...
                    BorderSizePixel = 0, ZIndex = 353, Parent = row,
                })
                ...
                local rowLabel = New("TextLabel", {
                    ...
                    Size = UDim2.new(1, -28, 1, 0), TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 353, Parent = row,
                })
                local hit = New("TextButton", {
                    Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = "", ZIndex = 354, Parent = row,
                })
                ...
                hit.MouseButton1Click:Connect(function()
                    ...
                    rowLabel.TextColor3 = sel[item] and T.Text or T.TextSub
                    dbtn.Text = SelText()
            ...
            task.defer(function()
                menuScroll.CanvasSize = UDim2.new(0, 0, 0, menuListLayout.AbsoluteContentSize.Y + 8)
            ...
            dbtn.MouseButton1Click:Connect(function()
                ...
                if isOpen then
                    ...
                    menuScroll.CanvasPosition = Vector2.new(0, 0)
                    task.defer(function()
                        local layout = menuScroll:FindFirstChildWhichIsA("UIListLayout")
                        if layout then
                            menuScroll.CanvasSize = UDim2.new(0, 0, 0, layout.AbsoluteContentSize.Y + 8)
                        end
                    end)
                else
                    menu.Visible = false
            ...
            local closeConn
            local function bindClose()
                if closeConn then return end
                closeConn = UIS.InputBegan:Connect(function(i)
                    if i.UserInputType ~= Enum.UserInputType.MouseButton1 or not isOpen then return end
                    task.defer(function()
            ...
            dbtn.MouseButton1Click:Connect(function() task.defer(bindClose) end)
            ...
            local ctrl = {
                ...
                Set = function(_, arr)
                    ...
                    dbtn.Text = SelText()
                    for _, row in ipairs(menuScroll:GetChildren()) do
                        if row:IsA("Frame") then
                            local lbl = row:FindFirstChildWhichIsA("TextLabel")
                            local chkFrame = row:FindFirstChildWhichIsA("Frame")
                            if lbl and chkFrame then
                                local txt = lbl.Text
                                chkFrame.BackgroundColor3 = sel[txt] and T.Accent or T.Elevated
                                lbl.TextColor3 = sel[txt] and T.Text or T.TextSub
                            end
                        end
                    end
                    if opts.Callback then opts.Callback(ctrl.Get()) end
                end,
                Frame = card
            }
            RegEl(opts.Id, ctrl)
        ...
        function TabObj:AddInput(o) return TabObj.AddTextInputToFrame(self, o, scrollFrame, allElements) end
        ...
        function TabObj:AddSubTab(stOpts)
            ...
            function SubTabObj:AddParagraph(o) return TabObj.AddParagraphToFrame(nil, o, stFrame, subEls) end
            function SubTabObj:AddResetButton(o) return TabObj.AddResetButtonToFrame(nil, o, stFrame, subEls) end
        ...
        -- XanBar-style aliases
        TabObj.Section = TabObj.AddSection
        TabObj.Label = TabObj.AddLabel
        TabObj.Divider = TabObj.AddDivider
        TabObj.Paragraph = TabObj.AddParagraph
...
-- Quick notify (XanBar-style): NexusLib:QuickNotify("Done!", 2)
function NexusLib:QuickNotify(text, duration)
    self:Notify({ Title = "", Message = tostring(text), Duration = duration or 3, Type = "Info" })
end

-- ─── Quick (one-liner: window + first tab) ───────────────────────────────────
function NexusLib:Quick(title, tabName, icon)
    title = title or "Nexus"
    tabName = tabName or "Main"
    icon = icon or ""
    local Win = self:CreateWindow({ Title = title, Subtitle = "v1.0" })
    local Tab = Win:AddTab({ Name = tabName, Icon = icon })
    return Win, Tab
end
