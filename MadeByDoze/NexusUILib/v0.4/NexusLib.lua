...
function NexusLib:CreateWindow(opts)
    ...
    local contentArea = New("Frame", {
        ...
        BackgroundTransparency = 1, BorderSizePixel = 0, ClipsDescendants = true, Parent = win,
    })
    ...
    function WindowObj:AddTab(tOpts)
        ...
        function TabObj.AddDropdownToFrame(_, opts, tf, elList)
            ...
            local menu = New("Frame", {
                Size = UDim2.new(0,142,0,menuH), Position = UDim2.new(1,-150,1,4),
                BackgroundColor3 = T.Surface, BorderSizePixel = 0,
                ZIndex = 30, ClipsDescendants = true, Visible = false, Parent = card,
            ...
            for _, item in ipairs(items) do
                local ib = New("TextButton", {
                    ...
                    Size = UDim2.new(1,0,0,27), BorderSizePixel = 0, ZIndex = 31, Parent = menu,
                }); Corner(4, ib)
            ...
            dbtn.MouseButton1Click:Connect(function() isOpen = not isOpen; menu.Visible = isOpen end)
            AttachTip(card, opts.Tooltip); AttachCtx(card, opts.Context)
            local ctrl = {
        ...
        function TabObj.AddMultiDropdownToFrame(_, opts, tf, elList)
            ...
            local menu = New("Frame", {
                Size = UDim2.new(0,142,0,menuH), Position = UDim2.new(1,-150,1,4),
                BackgroundColor3 = T.Surface, BorderSizePixel = 0,
                ZIndex = 30, Visible = false, Parent = card,
            }); Corner(7, menu); Stroke(T.BorderLight, 1, menu); Pad(3,3,3,3,menu); List(menu, nil, 1)
        ...
        function TabObj.AddColorPickerToFrame(_, opts, tf, elList)
            ...
            local panel = New("Frame", {
                Size = UDim2.new(0,215,0,182), Position = UDim2.new(1,-225,1,5),
                BackgroundColor3 = T.Surface, BorderSizePixel = 0, ZIndex = 25, Visible = false, Parent = card,
            ...
            local function Upd()
                color = Color3.fromHSV(h,s,v); preview.BackgroundColor3 = color
            ...
            hit.MouseButton1Click:Connect(function() isOpen = not isOpen; panel.Visible = isOpen end)
            AttachTip(card, opts.Tooltip); AttachCtx(card, opts.Context)
            local ctrl = {
