--[[
╔═══════════════════════════════════════════════════════╗
║           NexusLib v0.3 - Roblox UI Library           ║
...
function NexusLib:CreateWindow(opts)
    ...
    local function ActivateTab(entry)
        ...
        for _, t in ipairs(allTabs) do
            ...
            t.Container.Visible = false  -- one frame hides everything for this tab
        end
        ...
        Tween(entry.Container, { Position = UDim2.new(0, 0, 0, 0) }, SPRING)
        activeTab = entry
    end
    ...
    function WindowObj:AddTab(tabOpts)
        ...
        -- ── Per-tab container ─────────────────────────────────────────────────
        -- This single frame owns the search bar, sub-tab bar, and all content.
        -- Flipping container.Visible is the ONLY thing needed to show/hide a tab.
        ...
        -- Search bar (lives inside container)
        ...
        -- Main content scrollframe (inside container, below search)
        ...
        local function InitSubTabs()
            ...
            -- Push search bar down, hide regular frame (sub-tab frames take over)
            searchBox.Position = UDim2.new(0, 8, 0, 40)
            frame.Visible      = false
            frame.Size         = UDim2.new(1, 0, 0, 0)

            -- Sub-tab pill bar inside container
            subTabBar = New("Frame", {
                Size             = UDim2.new(1, -16, 0, 26),
