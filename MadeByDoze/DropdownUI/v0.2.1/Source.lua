...
function Library:SetTheme(themeName)
    ...
    for _, gui in pairs(Services.CoreGui:GetChildren()) do
        if gui:IsA("ScreenGui") and string.find(gui.Name, "CustomUI_") then
            -- Update main frames
            for _, frame in pairs(gui:GetDescendants()) do
                if frame:IsA("Frame") then
                    if frame.Name == "MainFrame" then
                        frame.BackgroundColor3 = Config.MainColor
                    elseif frame.Name == "TitleBar" or frame.Name == "DropdownFrame" or frame:FindFirstChild("Header") then
                        frame.BackgroundColor3 = Config.SecondaryColor
                    elseif frame.Name == "AccentLine" then
                        frame.BackgroundColor3 = Config.AccentColor
                    elseif frame.Name == "ContentContainer" or frame.Name == "Content" or frame.Name == "DropdownContent" then
                        -- These stay transparent or use MainColor
                    elseif frame.Name == "Dropdown_" or string.find(frame.Name, "Dropdown_") then
                        frame.BackgroundColor3 = Config.SecondaryColor
                    end
                    
                    -- Update specific element types
                    if frame.Name == "Button" or frame.Name == "Toggle" or frame.Name == "Slider" or 
                       frame.Name == "ColorPicker" or frame.Name == "Selector" or frame.Name == "Keybind" or
                       frame.Name == "Section" or frame.Name == "InfoBox" or frame.Name == "ProgressBar" or
                       frame.Name == "MultiToggle" then
                        if frame:FindFirstChild("Label") or frame:FindFirstChild("Button") then
                            frame.BackgroundColor3 = Config.SecondaryColor
                        end
                    end
                elseif frame:IsA("TextLabel") or frame:IsA("TextButton") or frame:IsA("TextBox") then
                    -- Update text colors
                    if frame.TextColor3 ~= Config.AccentColor then -- Don't change accent colored text
                        frame.TextColor3 = Config.TextColor
                    end
                    
                    -- Update specific elements
                    if frame.Name == "Arrow" or string.find(frame.Text or "", "▼") then
                        frame.TextColor3 = Config.AccentColor
                    end
                end
            end
        end
