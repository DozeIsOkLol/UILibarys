...
function NovaUI:createWindow(config)
    ...
    local main = Instance.new("Frame", nova.screenGui); window.mainFrame = main
    ...
    local header = Instance.new("Frame", main); header.Size, header.BackgroundColor3 = UDim2.new(1,0,0,40), nova.theme.Primary
    local title = Instance.new("TextLabel", header); title.Size, title.Position, title.BackgroundTransparency = UDim2.new(1,-50,1,0), UDim2.new(0,15,0,0), 1; title.Font, title.Text, title.TextColor3, title.TextXAlignment, title.TextSize = Enum.Font.GothamBold, config.title, nova.theme.Text, Enum.TextXAlignment.Left, 16
    local tabContainer = Instance.new("ScrollingFrame", main); tabContainer.Size, tabContainer.Position, tabContainer.BackgroundColor3 = UDim2.new(0,140,1,-40), UDim2.new(0,0,0,40), nova.theme.Primary; tabContainer.BorderSizePixel, tabContainer.ScrollBarThickness = 0, 0; Instance.new("UIListLayout", tabContainer).Padding = UDim.new(0,5); Instance.new("UIPadding", tabContainer).PaddingTop = UDim.new(0,10)
    local contentContainer = Instance.new("Frame", main); contentContainer.Size, contentContainer.Position, contentContainer.BackgroundTransparency = UDim2.new(1,-140,1,-40), UDim2.new(0,140,0,40), 1
    ...
    function window:createTab(name)
        ...
        function tab:addButton(text, callback)
            local frame = Instance.new("Frame", content); frame.Size, frame.BackgroundColor3 = UDim2.new(1,0,0,35), nova.theme.Primary; Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 6)
            local textLabel = Instance.new("TextLabel", frame); textLabel.Size, textLabel.BackgroundTransparency = UDim2.new(1,0,1,0), 1; textLabel.Text, textLabel.Font, textLabel.TextColor3, textLabel.TextSize = text, Enum.Font.Gotham, nova.theme.Text, 14
            local button = Instance.new("TextButton", frame); button.Size, button.BackgroundTransparency, button.Text = UDim2.new(1,0,1,0), 1, ""; button.MouseEnter:Connect(function() tween(frame, {BackgroundColor3 = nova.theme.Accent}) end); button.MouseLeave:Connect(function() tween(frame, {BackgroundColor3 = nova.theme.Primary}) end); if callback then button.MouseButton1Click:Connect(callback) end; return frame
        end
        ...
        function tab:addToggle(text, default, callback)
            ...
            local button = Instance.new("TextButton", container); button.Size, button.BackgroundTransparency, button.Text = UDim2.new(1,0,1,0), 1, ""; button.MouseButton1Click:Connect(function() toggled = not toggled; tween(switch, {BackgroundColor3 = toggled and nova.theme.Accent or nova.theme.Secondary}); tween(knob, {Position = toggled and UDim2.new(1,-18,0.5,-8) or UDim2.new(0,2,0.5,-8)}); if callback then pcall(callback, toggled) end end); return container
        end
