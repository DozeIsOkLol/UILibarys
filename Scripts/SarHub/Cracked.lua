...
local ok, err = pcall(function()
    MeisterUI = loadstring(game:HttpGet(
        "https://raw.githubusercontent.com/DozeIsOkLol/UILibarys/refs/heads/main/Scripts/SarHub/UI.lua"
    ))()
end)
if not ok then
...
local Window = MeisterUI:CreateWindow({
    Name     = "SARHUB BY SARpastes | Cracked and key removed by EvolEzod",
...
-- Custom text button in Misc tab to show credit
MiscTab:CreateButton({
    Name = "Cracked and key removed by EvolEzod",
    Callback = function()
        MeisterUI:Notify({
            Title   = "EvolEzod",
            Content = "Enjoy the script without keys!",
            Duration = 3
        })
    end
})
...
-- Startup Notification with Credit
MeisterUI:Notify({
    Title   = "SARHUB Loaded",
    Content = "Cracked and key removed by EvolEzod. Press INSERT to toggle menu.",
    Duration = 6
})
