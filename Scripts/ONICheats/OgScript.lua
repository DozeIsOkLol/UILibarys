...
local AUTH_FILE = "ONICheats_AuthToken.txt"

local ACCESS_CONFIG = {
	Password = "itheardus",
	CommunityGroupId = 888454071,
	CommunitySlug = "ONICheats",
}
...
local function getCommunityUrl()
	local groupId = tonumber(ACCESS_CONFIG.CommunityGroupId) or 0
	if groupId <= 0 then
		return nil
	end
	local slug = tostring(ACCESS_CONFIG.CommunitySlug or "community"):gsub("[^%w%-]+", "-")
	return "https://www.roblox.com/communities/" .. groupId .. "/" .. slug
end

local function getAuthToken()
	local hash = 5381
	for i = 1, #tostring(ACCESS_CONFIG.Password) do
		hash = (hash * 33 + string.byte(tostring(ACCESS_CONFIG.Password), i)) % 2147483647
	end
	return "ONI_" .. tostring(hash)
end

local function isAccessGranted()
	if not hasFileSystem or not isfile(AUTH_FILE) then
		return false
	end
	local ok, saved = pcall(function()
		return readfile(AUTH_FILE)
	end)
	return ok and saved == getAuthToken()
end

local function saveAccessGranted()
	if not hasFileSystem or not writefile then
		return
	end
	pcall(function()
		writefile(AUTH_FILE, getAuthToken())
	end)
end

local function copyCommunityLink()
	local url = getCommunityUrl()
	if not url then
		return false, "Community link is not configured yet."
	end
	if not setclipboard then
		return false, "Clipboard not supported. Copy manually:\n" .. url
	end
	...
	if not copied then
		return false, "Failed to copy. Copy manually:\n" .. url
	end
	return true, "Copied to clipboard — paste in your browser to get the password."
end

local PasswordGateGui = nil

local function destroyPasswordGate()
	if PasswordGateGui then
		PasswordGateGui:Destroy()
		PasswordGateGui = nil
	end
end

local function showPasswordGate(onSuccess)
	...
	local playerGui = LocalPlayer:FindFirstChild("PlayerGui") or LocalPlayer:WaitForChild("PlayerGui", 20)
	if not playerGui then
		warn("[ONICheats] PlayerGui missing - cannot show password gate.")
		return
	end
	...
	PasswordGateGui = Instance.new("ScreenGui")
	PasswordGateGui.Name = "ONICheats_Access"
	PasswordGateGui.ResetOnSpawn = false
	PasswordGateGui.IgnoreGuiInset = true
	PasswordGateGui.DisplayOrder = 10000002
	PasswordGateGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	PasswordGateGui.Enabled = true
	PasswordGateGui.Parent = playerGui
	...
	panel.Parent = PasswordGateGui
	Instance.new("UICorner", panel).CornerRadius = UDim.new(0, 10)
	local panelStroke = Instance.new("UIStroke")
	panelStroke.Color = Color3.fromRGB(220, 20, 40)
	...
	headerAccent.BackgroundColor3 = Color3.fromRGB(220, 20, 40)
	headerAccent.BorderSizePixel = 0
	headerAccent.Parent = panel

	local gateLogo = Instance.new("ImageLabel")
	gateLogo.Name = "ONILogo"
	gateLogo.Size = UDim2.new(0, 40, 0, 40)
	gateLogo.Position = UDim2.new(0, 12, 0, 14)
	gateLogo.BackgroundTransparency = 1
	gateLogo.ScaleType = Enum.ScaleType.Fit
	gateLogo.ImageTransparency = 0
	gateLogo.Parent = panel
	...
	title.RichText = true
	title.Text = '<font color="rgb(220,20,40)">ONI</font><font color="rgb(255,255,255)">Cheats</font> Access'
	...
	credit.Position = UDim2.new(0, 12, 0, 60)
	credit.BackgroundTransparency = 1
	credit.Text = "by JT2B — enter password to continue"
	credit.TextColor3 = Color3.fromRGB(140, 140, 145)
	...
	task.defer(function()
		local logoLoaded = applyLogoImage(gateLogo)
	...
	passwordBox.Size = UDim2.new(1, -24, 0, 34)
	passwordBox.Position = UDim2.new(0, 12, 0, 96)
	passwordBox.BackgroundColor3 = Color3.fromRGB(22, 22, 26)
	passwordBox.TextColor3 = Color3.fromRGB(255, 255, 255)
	passwordBox.PlaceholderText = "Password"
	passwordBox.PlaceholderColor3 = Color3.fromRGB(120, 120, 125)
	passwordBox.Font = Enum.Font.GothamMedium
	passwordBox.TextSize = 13
	passwordBox.ClearTextOnFocus = false
	passwordBox.Text = ""
	passwordBox.Parent = panel
	Instance.new("UICorner", passwordBox).CornerRadius = UDim.new(0, 4)
	Instance.new("UIStroke", passwordBox).Color = Color3.fromRGB(45, 45, 50)

	local statusLabel = Instance.new("TextLabel")
	statusLabel.Size = UDim2.new(1, -24, 0, 18)
	statusLabel.Position = UDim2.new(0, 12, 0, 136)
	...
	unlockBtn.Size = UDim2.new(1, -24, 0, 34)
	unlockBtn.Position = UDim2.new(0, 12, 0, 164)
	unlockBtn.BackgroundColor3 = Color3.fromRGB(220, 20, 40)
	...
	Instance.new("UIStroke", unlockBtn).Color = Color3.fromRGB(255, 70, 90)

	local communityBtn = Instance.new("TextButton")
	communityBtn.Size = UDim2.new(1, -24, 0, 34)
	communityBtn.Position = UDim2.new(0, 12, 0, 208)
	communityBtn.BackgroundColor3 = Color3.fromRGB(28, 28, 34)
	communityBtn.Text = "COPY COMMUNITY LINK"
	communityBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
	communityBtn.Font = Enum.Font.GothamBold
	communityBtn.TextSize = 11
	communityBtn.Parent = panel
	Instance.new("UICorner", communityBtn).CornerRadius = UDim.new(0, 4)
	Instance.new("UIStroke", communityBtn).Color = Color3.fromRGB(45, 45, 50)

	local linkLabel = Instance.new("TextLabel")
	linkLabel.Size = UDim2.new(1, -24, 0, 34)
	linkLabel.Position = UDim2.new(0, 12, 0, 250)
	...
	local securityNotice = Instance.new("TextLabel")
	securityNotice.Size = UDim2.new(1, -24, 0, 52)
	securityNotice.Position = UDim2.new(0, 12, 0, 292)
	securityNotice.BackgroundTransparency = 1
	securityNotice.TextWrapped = true
	securityNotice.TextYAlignment = Enum.TextYAlignment.Top
	securityNotice.Font = Enum.Font.GothamMedium
	securityNotice.TextSize = 9
	securityNotice.TextXAlignment = Enum.TextXAlignment.Left
	securityNotice.TextColor3 = Color3.fromRGB(255, 145, 95)
	securityNotice.Text = "Do not share the password. If it is leaked, your client will stop working and you must join the community to regain access. Every user needs to get access from the group."
	securityNotice.Parent = panel

	local communityUrl = getCommunityUrl()
	if communityUrl then
		linkLabel.Text = "Password is on the community page. Press the button to copy the link:\n" .. communityUrl
		linkLabel.TextColor3 = Color3.fromRGB(150, 150, 155)
	else
		linkLabel.Text = "Set ACCESS_CONFIG.CommunityGroupId in the script to show your Roblox community link."
		linkLabel.TextColor3 = Color3.fromRGB(255, 170, 80)
	end

	local function tryUnlock()
		local entered = passwordBox.Text
		if entered == tostring(ACCESS_CONFIG.Password) then
			saveAccessGranted()
			destroyPasswordGate()
			onSuccess()
			return
		end
		statusLabel.Text = "Wrong password. Copy the community link to get it."
		statusLabel.TextColor3 = Color3.fromRGB(255, 120, 80)
	end
	...
	communityBtn.MouseButton1Click:Connect(function()
		local ok, msg = copyCommunityLink()
		statusLabel.Text = msg
		statusLabel.TextColor3 = ok and Color3.fromRGB(120, 200, 255) or Color3.fromRGB(255, 120, 80)
		if ok then
			notify("ONICheats", "Community link copied to clipboard.", 3)
		end
	end)
end
...
local Credit = Instance.new("TextLabel")
Credit.Size = UDim2.new(0, 52, 1, 0)
...
showPasswordGate(safeLaunchONICheats)
